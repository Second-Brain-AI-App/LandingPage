    ...
    groups: [
      ...
      },
      {
        ...
        items: [
          ...
          'Critical items show a "🔴 Critical" chip inline on the card, separate from the deadline state, with no duplicate red dots when an item is both Critical and Overdue. Thanks reddit user Damoet.',
          'Widget critical items render distinctly with a red triangle, red border, and faint red background. Thanks reddit user Damoet.',
          'Critical notifications prefix titles with ⚠ so the severity is visible on Apple Watch and lock screen. Thanks reddit user Damoet.',
          'Action detail sheet redesigned with a clearer hierarchy: title → content → labels → primary actions → corrections → delete.',
          'Memory detail sheet redesigned with focal content treatment — memories now read like reference material, not tasks.',
          ...
          'Classifier prompt updated with explicit guidance for Critical / Normal / Low importance, dramatically reducing under-labeling of critical items. Thanks reddit user Damoet.',
        ],
      },
      {
        type: 'fixes',
        items: [
          ...
          'Reminders stay in sync when you edit a task. Changing priority, deadline, effort, or type — and updating your daily reminder times — no longer leaves stale or wiped reminders in the queue.',
          'Reminders fire at the time you actually said. "By 8pm" now pings at 8pm, not at noon or at the start of the evening.',
